	...
	i(0)}([function(t,i)
		{
			...
			initialize:function(){
				this.activeUsers=0,gapi.analytics.auth.once("signOut",
			...
			pollActiveUsers_:function(){var t=this.get(),
			...
			({
				ids:t.ids,metrics:"rt:activeUsers"
			}).then(function(t){
				Keith_GA.set_data_news(t.result.rows[0]["0"]);
				var e=t.result,
			...
			{activeUsers:this.activeUsers,delta:t}),
			t>0 ? this.emit("increase",{ activeUsers:this.activeUsers,delta:t }) :
			...
			handleSignIn_:function(){this.pollActiveUsers_(),
			gapi.analytics.auth.once("signOut",this.handleSignOut_.bind(this))
		},
	template:'<div class="ActiveUsers" style="height: 100%; width: 81%; border-radius: 4px; margin-left: 1%;"><center><b class="ActiveUsers-value"></b>News Users Right Now</center></div>'})})}]);
